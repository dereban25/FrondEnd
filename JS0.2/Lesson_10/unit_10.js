...
let ar3 = [13, 24, 23, 'test', true, false]
...
function f8() {
	document.querySelector('.out-8').innerHTML = `${ar8[3] = '3.14'}-${ar8[4] = 17}-${ar8[6] = 5}`;
...
function f9() {
	...
	for (let i = 0; i <= ar9.length - 1; i++) {
		out9.innerHTML = ar9[i];
	}
}
...
function f10() {
	document.querySelector('.out-10').innerHTML = parseInt(`${ar10[0]}`) + parseInt(`${ar10[ar10.length - 1]}`);
}
...
// Task 11
// Напишите функцию, которая меняет местами второй (индекс 2)
// и четвертый (индекс 4) элемент массива ar11 и выводит его в out-11. Разделитель - пробел.
...
function f17() {
	...
	document.querySelector('.out-17').innerHTML = count.length;
}
...
function f19() {
	...
	for (let i = 0; i < ar19.length; i++) {
		if (ar19[i] < min)
			min = ar19[i];
		document.querySelector('.out-19').innerHTML = `${i}`;
	}
