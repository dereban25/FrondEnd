...
// Task 8.
// Создайте div.out-8 и кнопку .b-8.
// При нажатии кнопки создавайте внутри div.out-8 элемент input.i-81 и кнопку .b-81 (innerHTML).
...
function f8() {
	document.querySelector('.out-8').innerHTML = '<input type="text" class="i-81" value="I enter button 81">' +
...
// Task 9
//Создайте один input(radio).r-9  и button.b-9 -
// при нажатии на button если radio.r-9 выбран (активен, checked)
// выводите в .out-9 - value прописанное в r-9, либо 0 если не активен.

function f9() {
	let chR = document.querySelector('.r-9')
	if (chR.checked === true) {
		document.querySelector('.out-9').innerHTML = chR.value;
	} else {
...
function f10() {
	let colorInput = document.querySelector('.i-10');
	const colorOut = document.querySelector('.out-10');
	console.log(colorInput.value);
	colorOut.style.background = colorInput.value;
}
...
// Task 13
//  Создайте input(range).i-13. При изменении положения ползунка i-13 выводите значение в out-13.
//  Для события используйте oninput.
...
function f15() {
	...
	let texArea = document.querySelector('.t-15');
	let out15 = document.querySelector('.out-15');
	texArea.value = in15.value;
	out15.innerHTML = in15.value;
}
...
// Task 18
// Создайте select.s-18 и input.i-18.
// Добавьте на select событие onchange, при наступлении которого запускается функция f18.
...
function f19(event) {
	...
	document.querySelector('.out-19').innerHTML = text.value + ' ' + pass.value;
